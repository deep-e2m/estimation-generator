...
- When the requirements or any attached Source document / SOW state explicit **Estimated Total Effort** (e.g. "120–140 hours") or **Estimated Timeline** (e.g. "3–4 weeks"), treat those as authoritative and match them exactly in your quote; do not substitute your own range.
- If the client's additional instructions say not to use competitor or opposition company content as their website content, do not use any reference URL (scraped) content as the client's own website content; use references only for inspiration where appropriate.
...
**Reference URLs — access issues (CRITICAL)**: Do NOT mention blocked URLs, 403/401 errors, access denied, scraping failures, "inaccessible", "could not be accessed", or "unable to fetch" anywhere in the estimation (Project Overview, Website Structure, Development Approach, or any section). If reference URLs could not be fetched or analyzed, estimate based on the project description and other available context. Proceed with the information you have—never explain access failures or inaccessibility of design links.
...
- **Timeline and effort**: When the brief includes a "Source document / SOW" (or similar) section with explicit **Estimated Total Effort** (e.g. "120–140 hours") or **Estimated Timeline** (e.g. "3–4 weeks", "approximately 15–18 business days"), treat those as authoritative. Your total_hours and estimated_effort_timeline MUST match those figures; do not substitute your own range. When the SOW does not state hours or timeline, derive from the described scope (deliverables, phases, complexity), then derive business days as total_hours ÷ 8 (or a realistic rate). Output format: "Estimated Total Effort: X–Y hours (approximately Z business days)" where X–Y and Z come from the SOW when provided, otherwise from your scope-based estimate. The timeline must be accurate, defensible, and consistent (Z ≈ total_hours ÷ 8).
...
- **Development Approach ↔ products**: When Website Structure lists product-related pages (Products Overview, Individual Product Page, etc.), the Development Approach MUST include the appropriate plugin: **WooCommerce** when the reference or brief implies product purchasing, cart, checkout, or e-commerce; **ACF Pro** with Custom Post Types when the brief explicitly excludes e-commerce (content-only product display). Do not list product pages without a corresponding plugin in Development Approach. If the reference URL content (Theme/plugin implications, Design inference, or scraped text) mentions WooCommerce, include it in Development Approach and do NOT add "E-commerce functionality is not included" to Exclusions unless the brief explicitly excludes it.
"""
...

{
  "estimation_outcomes": {
    ...
    "development_approach": "Full section 6 content: Development approach, tech stack, responsive, QA. The Development Approach section MUST list the exact theme, page builder, and every plugin that will be used, each by name with its official URL in the form **Name** (URL: https://...). When the brief or reference content includes product pages (Products Overview, Individual Product Page) and implies WooCommerce or e-commerce, include **WooCommerce** (URL: https://woocommerce.com/). Do not omit WooCommerce when the reference suggests it. Do not write generic phrases like 'a form plugin' or 'WordPress and WooCommerce' without naming the theme, page builder, and key plugins with URLs. Use the locked-in and recommended tools provided in the WordPress Stack section when present; otherwise use company-approved defaults and include their URLs.",
    "estimated_effort_timeline": "Full section 7: Use BULLETS. Put each line on its own bullet starting with '- '. Include '- Estimated Total Effort: X–Y hours (approximately Z business days)' and '- Estimated Timeline: X–Y weeks from project kickoff' (or similar). If the brief/SOW states a duration, add '- *Note: Client/SOW referenced timeline: ...*' as a bullet.",
...
- **Format by section (CRITICAL)**:
  - **project_overview**: Paragraph format only. Write 2–4 flowing sentences. Do NOT use bullets or list items.
  - **website_structure, development_approach, estimated_effort_timeline, assumptions, exclusions**: Bullet format only. Each section starts with 1–2 optional intro sentences; then EVERY substantive item MUST be on its own line starting with "- " (e.g. "- Item one."). Do NOT write long paragraphs for these sections. Each bullet must be a complete sentence.
...
- Website Structure: Start with a brief intro (1–2 sentences); then use BULLETS ONLY: each page/section on its own line starting with "- " (e.g. "- Home – Main landing page."). When the brief includes "Pages and frames (full sitemap)" (Figma or design reference), list every item from that list by exact name—never replace with generic "Home, About, Services, Products, Contact, Privacy Policy" when the reference gives different names. Use "Page name – Short description" format per bullet.
- Assumptions and Exclusions: Each section starts with a short intro. Then use BULLETS ONLY: every item on its own line starting with "- ", and each must be a full sentence (e.g. "- The brand book will be finalized before design begins." and "- Advanced recommendation engines are out of scope."). No paragraphs, no sentence fragments, shorthand, or single-word bullets.
...
- Development Approach: Use BULLETS ONLY. Start with 1–2 optional intro sentences; then each theme, page builder, plugin, and implementation detail on its own line starting with "- ". List every theme, page builder, and plugin by name with URL. Do not write a single paragraph; use bullet items for clarity.
"""
    messages.append({"role": "user", "content": user_content})
    return messages
...
- **Pages and frames (full sitemap)** — CRITICAL: List EVERY page, frame, or screen name visible in the design. Look for frame titles in the left panel, canvas labels, page names, section headers. Output a bullet list like: "Pages and frames (full sitemap): Home, About Us, Services, Products Overview, Individual Product Page, Contact, Footer." Use the exact names shown. This list is used for the Website Structure section—do not substitute generic names (Home, About, Contact) when the design shows different names (e.g. "Location overview page", "Individual Location Pages", "Warranty Information").
...
- **Theme/plugin implications**: What would typically be needed in WordPress to replicate this: e.g. "Elementor Pro or equivalent for full-width sections and custom blocks; contact form plugin; **WooCommerce** for product pages" or "Simple theme + Elementor free; one contact form; standard blog layout; no e-commerce." **Products rule**: When the design shows "Products Overview", "Individual Product Page", product listings, cart/checkout, pricing, or add-to-cart—explicitly state "WooCommerce" in Theme/plugin implications. Only say "no e-commerce" or "ACF/custom post type for products" when the design clearly shows content-only product info (brochure-style) with no cart/checkout. Be specific so the estimate can name theme and plugins.
...
3. **Theme/plugin implications**: What would typically be needed in WordPress to replicate this: page builder (e.g. Elementor Pro vs free), form plugin if contact/forms appear, e-commerce (yes/no), any special needs (video, gallery, multi-section landing). **Products rule**: When the content mentions "Products Overview", "Individual Product Page", product listings, cart, checkout, pricing, or WooCommerce—explicitly state "WooCommerce" in Theme/plugin implications. Only say "no e-commerce" or "ACF for product info only" when the content clearly indicates content-only product display with no purchasing. Be specific so the estimate can name theme and plugins.
