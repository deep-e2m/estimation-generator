...
class DocxExportService:
    ...
    def _add_scope_of_work(self, data: QuoteExportData) -> None:
        ...
        # Try to extract scope sections from the content
        scope_sections = self._extract_scope_sections(content)

        if scope_sections:
            for section_title, section_content in scope_sections:
                self._document.add_heading(section_title, level=2)
                self._add_formatted_content(section_content)
        else:
            # If no structured sections found, add content as-is
            self._add_formatted_content(content)

    def _extract_scope_sections(
        self, content: str
    ) -> list[tuple[str, str]]:
        """
        Extract structured sections from quote content.
    ...
    def _extract_hours_breakdown(self, content: str) -> list[dict[str, Any]]:
        ...
        # Pattern to match task/hours combinations
        patterns = [
